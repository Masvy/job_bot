...
from keyboards.user_keyboards import vacancies_kb2, \
     employments_vac_1, experience_vac, \
     education_vac, back_menu_kb, \
     questions_vac, employments_vac_2, true_false_kb2
from database.users import update_city, update_vacancies, update_employment, \
     ...
     update_status, update_question, update_access, read_access
...
async def request_question_2(callback: CallbackQuery,
    ...
    await callback.message.answer(text='На собеседование возьмите с собой '
                                  ...
                                  reply_markup=questions_vac)
    await state.clear()


@vacancies_router.callback_query(F.data == 'no_questions2')
async def respond_rejection2(callback: CallbackQuery,
    ...
    access = await read_access(callback.from_user.id,
                               session_maker=session_maker)
    if 1 <= access < 3:
        await update_access(callback.from_user.id, access=2, session_maker=session_maker)
...
@vacancies_router.callback_query(F.data == 'questions2',
                                 StateFilter(default_state))
...
async def answer_question2(message: Message, state: FSMContext,
    ...
    access = await read_access(message.from_user.id,
                               session_maker=session_maker)
    if 1 <= access < 3:
        await update_access(message.from_user.id, access=2, session_maker=session_maker)
